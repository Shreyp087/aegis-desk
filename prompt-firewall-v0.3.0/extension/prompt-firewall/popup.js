...
function renderEvents(events) {
  ...
  for (const event of events) {
    ...
    const injBadge = event.injectionDetected
      ? `<span class="injection-badge">🚨 INJECT</span>` : "";
    li.innerHTML = `
      <div class="event-top">
...
function drawSparkline(ledger) {
  ...
  const points      = scanEntries.slice(0, 30).reverse(); // oldest first
  ...
  const yAt = (r) => pad.t + gH - ((r - 0) / Math.max(1, maxR)) * gH;
